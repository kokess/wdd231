...
function buildCard(r) {
  ...
  card.innerHTML = `
    ...
    <div class="restaurant-card-body">
      <button class="lff-fav-btn" data-id="${r.id}" aria-label="${isFav ? 'Remove from' : 'Add to'} favourites" aria-pressed="${isFav}">
        ${isFav ? '❤️' : '🤍'}
      </button>
      <h3>${r.name}</h3>
      ...
      <button class="lff-card-btn" data-id="${r.id}">View Details</button>
    </div>
  `;

  // View details opens modal
  card.querySelector('.lff-card-btn').addEventListener('click', e => {
    e.stopPropagation();
    openModal(r);
  });
  ...
  if (list.length === 0) {
    grid.innerHTML = `
      <div class="no-results">
        ...
        <p class="no-results-sub">Try a different search term or filter.</p>
      </div>`;
...
function initFavToggle() {
  ...
  btn.addEventListener('click', () => {
    ...
    btn.classList.toggle('lff-btn-orange', showFavOnly);
    btn.classList.toggle('lff-btn-outline', !showFavOnly);
    applyFilters();
  });
}
...
function openModal(r) {
  ...
  modalBody.innerHTML = `
    ...
    <div class="lff-modal-stars" aria-label="Rating ${r.rating} out of 5">${starsHTML(r.rating)} <strong>${r.rating}</strong> / 5</div>
    <div class="lff-modal-meta">
    ...
    <p class="lff-modal-desc">${r.description}</p>
    <p class="lff-modal-specialty"><strong>Specialty:</strong> ${r.specialty}</p>
    <div class="lff-modal-tags lff-mb2">${tagsHTML}</div>
    <button class="lff-btn lff-btn-primary lff-w-full" id="modalFavBtn">
  ...
  overlay.classList.add('lff-open');
  closeBtn.focus();
...
function closeModal() {
  overlay.classList.remove('lff-open');
}
